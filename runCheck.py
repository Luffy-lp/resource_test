...
class Check():
    ...
    def check(self):
        ...
        if confresult is not True:
            self.report(False, "配置下载", self.bookchapter, "下载对白配置文件失败")
            return False
        ...
        if resourceResult is not True:
            self.report(False, "资源下载", self.bookchapter, "下载资源文件失败")
            return False
        ...
        self.getfile_name()  # 获取资源文件用于对比 555555555555555555555555555555555特殊处理
        self.contrast()  # 对比并给出结果
        return True
        # except Exception as e:
        #     self.report(False, "未知", self.bookchapter, e)
        #     return False
    ...
    def contrast(self):
                ...
                for part, file in parts.items():
                    ...
                    if result is False:
                        ...
                        if result is False:
                            ...
                            self.report(False, part+"资源", self.bookchapter, content, roleID=role_id)
        ...
        for role_id, fashionidlist in self.face_check_dir.items():
            for i in fashionidlist:
                if role_id not in self.role_resources_dir:
                    ...
                    self.report(False, "face文件", self.bookchapter, content, roleID=role_id)
                else:
                    k = i + ".png"
                    if k in self.role_resources_dir[role_id]:
                        content = "未找到{0}的文件".format(k)
                        self.report(False, "face资源", self.bookchapter, content, roleID=role_id)
                    else:
                        k1 = i + "_1" + ".png"
        ...
        for bg in self.bg_check_list:
            ...
            self.report(bgbool, "background资源", self.bookchapter, content)
        ...
        for bgm in self.bgm_check_list:
            ...
            self.report(bgmbool, "bgm资源", self.bookchapter, content)
        ...
        for sound in self.sound_check_list:
            ...
            self.report(soundbool, "sound资源", self.bookchapter, content)
            ...
            if chat["is_finished"]:
                ...
                if is_finished == "1":
                    ...
                    if self.is_finished > 1:
                        self.report(False, "is_finished数量", self.bookchapter, "is_finished=1数量超过1",
        ...
        if select_id and select_id != 0 and select_id != "0":
            ...
            if select_id in select_info:
                for select in select_info[select_id].values():
                    ...
                    #     mylog.error(content)
                    #     self.report(False, "face", self.bookchapter, content, roleID=role_id)
                    if select["goodstype"]:
                ...
                if search:
                    ...
                    self.report(False, "search_select_id", self.bookchapter, content, roleID=role_id, chat=chat["id"])
                else:
                    content = "未找到select_id{1}".format(select_id)
                    self.report(False, "select_id配置", self.bookchapter, content, roleID=role_id, chat=chat["id"])
            ...
            if select_id in select_info:
                ...
                for select in select_info[select_id].values():
                    ...
                    if not select["jump_chat_id"] > 0:
                        ...
                        # mylog.error(content)
                        self.report(False, "jump_chat_id", self.bookchapter, content, roleID=role_id, chat=chat["id"])
                    if select["goodstype"]:
                        if select["goodstype"] == "fashion":
                            self.role_fashion_dir[role_id].append(select["goodsid"])
                        else:
    ...
    def mail(self, chat):
        ...
        try:
            ...
            self.report(False, "mail", self.bookchapter, "邮件配置异常")
    ...
    def add_friend_id(self, chat):
        ...
        if chat["chat_type"] == 8 or chat["chat_type"] == 15 or chat["chat_type"] == 16 or chat["chat_type"] == 19 or \
            ...
            if friend_id and friend_id != "0":
                ...
                # mylog.error(content)
                self.report(False, "friend_id", self.bookchapter, content, chat=chat["id"])
